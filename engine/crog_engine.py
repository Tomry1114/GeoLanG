...
def train_with_grasp(train_loader, model, optimizer, scheduler, scaler, epoch, args):
    ...
    for i, data in enumerate(train_loader):
        ...
        image = data["img"]  # torch.Size([24, 3, 416, 416])
        text = data["word_vec"]  # torch.Size([24, 17])  >>  tensor([49406,  4646,   518,  5894, 20934,  7053, 49407,
        ins_mask = data["mask"]  # torch.Size([24, 416, 416])
        grasp_qua_mask = data["grasp_masks"]["qua"]  # torch.Size([24, 416, 416])
        grasp_sin_mask = data["grasp_masks"]["sin"]  # torch.Size([24, 416, 416])
        grasp_cos_mask = data["grasp_masks"]["cos"]  # torch.Size([24, 416, 416])
        grasp_wid_mask = data["grasp_masks"]["wid"]  # torch.Size([24, 416, 416])
        
        
        data_time.update(time.time() - end)
        # data
        image = image.cuda(non_blocking=True)  # 非阻塞的将cpu上的张量移至gpu上
        
        dp_image = data['depth'].cuda(non_blocking=True)
        # 添加 depth 通道维度
        dp_image = dp_image.unsqueeze(1)  # [B, 1, 480, 640]
        # 调整深度图像尺寸
        dp_image = F.interpolate(dp_image, size=(416, 416), mode='bilinear', align_corners=False)  # [B, 1, 416, 416]
        ...
        # forward
        with amp.autocast():  # 混合精度训练
            pred, target, loss, loss_dict = model(image, text, ins_mask, grasp_qua_mask, grasp_sin_mask, grasp_cos_mask, grasp_wid_mask, dp_image)
        ...
        scaler.scale(loss).backward()  # 缩放损失并计算梯度（避免混合精度训练过程中损失过小）
        if args.max_norm:
        ...
        iou, pr5 = trainMetricGPU(ins_mask_pred, ins_mask_target, 0.4, 0.5)
...
def validate_with_grasp(val_loader, model, epoch, args):
    ...
    for data in pbar:
        ...
        pred, target = model(image, text, ins_mask, grasp_qua_mask, grasp_sin_mask, grasp_cos_mask, grasp_wid_mask, dp_image)
        ...
        for idx in range(ins_mask_preds.shape[0]):
            ...
            ins_mask_pred = (ins_mask_pred > 0.4)
