...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int,
                 layers: int, heads: int, output_dim: int, out_indices=None):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        # charlie3go 05/27
        ...
        # charlie3go 05/27 设置要提取的中间层索引，默认提取所有 block 输出
        self.out_indices = out_indices if out_indices is not None else list(range(layers))
    ...
    # charlie3go 05/27
    def forward(self, x: torch.Tensor):
        x = self.forward_features_multiscale(x)  # 默认返回中间层
        return x

    # charlie3go 05/27
    # 获取中间层特征
    def interpolate_pos_encoding(self, x: torch.Tensor, num_patches_h: int, num_patches_w: int):
        """
        插值 positional embedding 到目标大小
        """
        N = num_patches_h * num_patches_w
        pos_embed = self.positional_embedding[1:]  # 去掉 class token 的 pos emb
        dim = pos_embed.shape[1]

        # 当前 pos_emb 对应的 grid size (e.g., 14x14)
        orig_size = int(math.sqrt(pos_embed.shape[0]))
        pos_embed = pos_embed.reshape(1, orig_size, orig_size, dim).permute(0, 3, 1, 2)

        # 使用双线性插值扩展到目标 grid size
        ...
        pos_embed = pos_embed.permute(0, 2, 3, 1).reshape(N, dim)
        return pos_embed

    def forward_features_multiscale(self, x: torch.Tensor):
        B, C, H, W = x.shape

        # Patch embedding
        x = self.conv1(x)  # shape = [*, width, h, w]
        _, _, ph, pw = x.size()
        x = x.flatten(2).transpose(1, 2)  # shape = [*, N_patches, width]
        ...
        pos_emb = self.interpolate_pos_encoding(x, num_patches_h, num_patches_w)
        x = x + torch.cat([self.positional_embedding[:1], pos_emb], dim=0)  # 加上 class token 的 pos_emb
        ...
        for i, blk in enumerate(self.transformer.resblocks):
            ...
            if i in self.out_indices:
                feat = x.permute(1, 0, 2)  # LND -> NLD
                # 去掉 class token 并 reshape 成图像格式
                feat = feat[:, 1:, :].permute(0, 2, 1)  # [B, D, N]
                feat = feat.unflatten(-1, (num_patches_h, num_patches_w))  # [B, D, H', W']
