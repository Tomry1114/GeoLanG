...
try:
    ...
    # fvcore is only used by the (optional) .flops() helpers; not needed for forward.
...
def selective_scan_torch(u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=True):
    """Pure-PyTorch reference selective scan (no custom CUDA kernel required).

    Matches the packed layout produced by ``cross_selective_scan``:
        u, delta : (Bsz, KD, L)      A : (KD, N)
        B, C     : (Bsz, K, N, L)    D : (KD,)   delta_bias : (KD,)
    Returns ``out`` of shape (Bsz, KD, L). Sequential O(L) scan — correct but slow;
    intended as a portable fallback so the VMamba forward runs on CPU / without the
    compiled selective_scan kernels.
    """
    dtype_in = u.dtype
    ...
    N = A.shape[1]
    K = B.shape[1]
    r = KD // K  # channels per scan-direction group sharing one (B, C)

    # expand grouped B, C to per-channel: (Bsz, KD, N, L)
    B = B.repeat_interleave(r, dim=1)
    C = C.repeat_interleave(r, dim=1)

    deltaA = torch.exp(torch.einsum('bdl,dn->bdln', delta, A))         # (b, KD, L, N)
    deltaB_u = torch.einsum('bdl,bdnl,bdl->bdln', delta, B, u)         # (b, KD, L, N)

    x = u.new_zeros((bsz, KD, N))
    ys = []
    for i in range(L):
        x = deltaA[:, :, i] * x + deltaB_u[:, :, i]                    # (b, KD, N)
        ys.append(torch.einsum('bdn,bdn->bd', x, C[:, :, :, i]))       # (b, KD)
    y = torch.stack(ys, dim=-1)                                        # (b, KD, L)
    if D is not None:
...
class SelectiveScanPyTorch(torch.autograd.Function):
    """Autograd shim exposing ``selective_scan_torch`` with the same ``.apply``
    signature as the CUDA SelectiveScan functions. Uses pytorch autograd (no manual
    backward), so it is differentiable but runs anywhere (CPU included)."""
    @staticmethod
    def apply(u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False,
              nrows=1, backnrows=1, oflex=True):
        return selective_scan_torch(u, delta, A, B, C, D, delta_bias, delta_softplus)
    ...
    def __initv2__(
        ...
        **kwargs,    
        ...
        FORWARD_TYPES = dict(
            ...
            # portable pure-pytorch fallback (no compiled selective_scan kernel needed)
            v2pt=partial(self.forward_corev2, force_fp32=True, SelectiveScan=SelectiveScanPyTorch),
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.classifier = nn.Sequential(OrderedDict(
            ...
            head=nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
        ))
